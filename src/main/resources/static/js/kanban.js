       ...
       // defaults
        ...
        function build_titles() {
            ...
            settings.titles.forEach(function (item, index, array) {
                var item = '<div style="background: '+settings.colours[index]+'" class="card-width ' + classes.kanban_board_title_class + '">' + '<p>'+item+'</p>' + '</div>';
                $this.find('.'+classes.kanban_board_titles_class).append(item);
            });
            $( "."+classes.kanban_board_titles_class ).sortable({
                connectWith: "."+classes.kanban_board_titles_class,
                containment: "."+classes.kanban_board_class,
        ...
        function build_blocks() {
            ...
            $( "."+classes.kanban_board_blocks_class ).sortable({
                connectWith: "."+classes.kanban_board_blocks_class,
                containment: "."+classes.kanban_board_class,
                placeholder: classes.kanban_board_item_placeholder_class,
                scroll: true,
                cursor: "move",
